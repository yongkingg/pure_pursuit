...
y_model = model[0] * x_model**2 + model[1] * x_model + model[2]
...
plt.plot(x_model, y_model, color='red', linewidth=1, label="RANSAC Fit: y = {:.2f}x^2 + {:.2f}x + {:.2f}".format(*model))

# 5. 그래프 설정
plt.title('Comparison of Original Data and RANSAC Result')
plt.xlabel('X')
plt.ylabel('Y')
plt.legend()
plt.grid(True)
